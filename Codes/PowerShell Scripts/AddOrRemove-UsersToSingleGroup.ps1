...
Foreach ($M in $members) {
    ...
    Try {
        ...
        #Remove-ADGroupMember -Identity $Groupname -Members $M.samaccountname -ErrorAction Stop
        $Property = [ordered]@{GroupName = $GroupName; samaccountname = $M.samaccountname; Message = 'UpdatedSuccessfully' }
    }
    Catch {
