...
Function Reload_menu_now {
    $menuOption = 0 #It's important to reset the variable
    LoadMenu
}

Function LoadMenu {

    [int] $menuOption = 0        #Resets the menu options for each time the menu loads
    ...
    Switch ( $menuOption ) {
        ...
        1 {
            ...
            Catch {
                #If something goes wrong a error message will be displayed
            ...
            If ($Answer -eq "n") {
                ...
                #if user didn't answer 'n' then he/she wishes then to start the network
                Try {
                    Write-Host ""
